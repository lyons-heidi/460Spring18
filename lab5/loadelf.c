...
int mylseek(int fd, int position, int how) // fd is dummy, only positon matters
{
...
int myread(int fd, char *buf, int nbytes) // fd is dummy; RW from fosset
{
  u32 i, lbk, blk, offset;

  lbk = foffset / BLKSIZE; offset = foffset % BLKSIZE;
  //printf("myread: fosset=%x blk=%x offset=%x\n", foffset, lbk, offset);
  if (lbk < 12)
    blk = iip->i_block[lbk];
  if (lbk >=12 && lbk < 256){
    getblk(iip->i_block[12], (char *)blkbuf);
    lbk -= 12;
    blk = blkbuf[lbk];
  } 
  // printf("blk=%x offset=%x\n", blk,offset);

  getblk(blk, tbuf);  // get a block into tbuf[ ];
  //for (i=0; i<8; i++)
  //  printf("%c| ", tbuf[i]);
  //printf("tbuf=%s\n", tbuf);
...
************************************************************/
  ...
  for (i=0; i<nn; i++){
   ...
   iip = ip; // set global iip for myread();

   // myread(fd, ebuf, 256, 1); // read elf header
   getblk(ip->i_block[0], ebuf); // read block 0 of file
   ...
   pgtable = (u32 *)(p->pgdir[2048] & 0xFFF00000); // get VA
   ...
   for (i=1, ph=aph; i <= phnum; ph++, i++){
       ...
       */
       /* MUST FIX this */
       mylseek(fd, (long)ph->offset, 0); // set foffset to ph->offset

       pn = ph->vaddr/0x100000; // convert vaddr to page number
       //printf("page=");
       count = 0; 

       addr = (char *)((u32)p->pgdir[pn] & 0xFFF00000);
 ...
 *************************************************************/
	 // printf("addr=%x ", addr);
       ...
       /* must fix this */         
       myread(fd, dbuf, BLKSIZE); // read by 1KB BLKSIZE
       //printf("load: %s\n", dbuf);  
       //copy(addr, buf);
       memcpy(addr, dbuf, BLKSIZE);  // 1K size since read by 1KB
             addr  += BLKSIZE;
             count += BLKSIZE;
             pn++;
       }
